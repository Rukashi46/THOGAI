...
import { handleReconcileRequest } from './api/reconcile.ts'
...
function aiApiPlugin(env: Record<string, string>): Plugin {
  return {
    ...
    configureServer(server) {
      ...
      // Reconciliation statement reader endpoint
      server.middlewares.use('/api/reconcile', async (req: IncomingMessage, res: ServerResponse) => {
        ...
        req.on('end', async () => {
          try {
            ...
            const apiKey = env.GEMINI_API_KEY || process.env.GEMINI_API_KEY || ''
            const model = env.GEMINI_MODEL || process.env.GEMINI_MODEL || 'gemini-3.6-flash'
            const result = await handleReconcileRequest(payload, apiKey, model)
            ...
            res.statusCode = 400
            res.setHeader('Content-Type', 'application/json')
            res.end(JSON.stringify({ error: 'INVALID_JSON', message: 'Invalid request.' }))
          }
        })
      })
    }
